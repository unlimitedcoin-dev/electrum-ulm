...
from electrum_ulm.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds
from electrum_ulm import bitcoin
from electrum_ulm.util import timestamp_to_datetime
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        ...
        try:
            ...
            self.app.show_info(_("Not a Unlimitedcoin URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_ulm.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Unlimitedcoin address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Unlimitedcoin Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_ulm.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Unlimitedcoin Request"))
